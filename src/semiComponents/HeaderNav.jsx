...
const HeaderNav = ({isHidden}) => {
  return (
    <>
      ...
      <LinkRouter to='/login' className={`${!isHidden ? "block" : "hidden" } text-white cursor-pointer border-2! border-gradient hover:border-white/45 background-gradient p-2 `}>
        Se connecter
      </LinkRouter>
      <a href="/login" className={`${!isHidden ? "hidden" : "block" } text-white cursor-pointer border-2! border-gradient hover:border-white/45 background-gradient p-2 `}>
        Se connecter
      </a>
    </>
  )
