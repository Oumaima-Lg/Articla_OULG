...
import { ArrowLeft, Heart, MessageCircle, Share2, MoreHorizontal, Bookmark, Trash2, Plus } from "lucide-react"
import { FaQuoteLeft, FaHeart, FaBookmark } from "react-icons/fa"
import { BiBook } from "react-icons/bi"
import { LoadingOverlay } from "@mantine/core"
import { useSelector } from "react-redux"
import { getMyPosts, likeArticle, unlikeArticle, deleteArticleWithUserId } from "../../services/ArticleService" // ✅ Import de deleteArticle
import { saveArticle, unsaveArticle } from "../../services/UserService"
...
const MyPosts = () => {
    ...
    const [deleteLoading, setDeleteLoading] = useState({}); // ✅ État pour les suppressions

    useEffect(() => {
        setLoading(true);
        getMyPosts(user.id)
            .then((data) => {
                console.log('Mes postes:', data);
            ...
            .catch((err) => {
                setLoading(false);
                console.error(err);
                errorNotification('Erreur', 'Impossible de charger vos postes');
            });
    }, [user.id]);
    ...
    const handleDelete = async (articleId) => {
        const article = posts.find(p => p.article.id === articleId);
        const articleTitle = article?.article.title || article?.article.content?.substring(0, 50) + '...';

        if (window.confirm(`Êtes-vous sûr de vouloir supprimer cet article ?\n\n"${articleTitle}"\n\nCette action est irréversible et supprimera également tous les likes et sauvegardes associés.`)) {
            setDeleteLoading(prev => ({ ...prev, [articleId]: true }));

            try {
                await deleteArticleWithUserId(articleId, user.id);

                // Retirer l'article de la liste locale
                setPosts(prev => prev.filter(poste => poste.article.id !== articleId));

                successNotification('Succès', 'Article supprimé avec succès');

                // Fermer le dropdown s'il était ouvert
                setShowDropdown(null);

            } catch (err) {
                console.error('Erreur lors de la suppression:', err);
                errorNotification('Erreur', err.response?.data?.error || 'Impossible de supprimer l\'article');
            } finally {
                setDeleteLoading(prev => ({ ...prev, [articleId]: false }));
            }
        }
    };
        ...
        <div className="min-h-screen p-4 md:p-6 bg-gradient-to-bl from-[#171717] from-55% to-[#4E3F59] to-100%">
            ...
            <div className="flex items-center justify-between mb-8">
                ...
                <div className="flex items-center gap-4">
                    <span className="text-[#A09F87] text-sm">{posts.length} article{posts.length !== 1 ? 's' : ''}</span>
                    <button
                        onClick={() => navigate("/articla/nouveauArticle")}
                        className="bg-[#A09F87] hover:bg-[#A09F87]/80 text-[#171717] p-2 rounded-lg transition-colors"
            ...
            <div className="max-w-2xl mx-auto space-y-6">
                {posts.length === 0 ? (
                    <div className="text-center py-12">
                        <BiBook className="w-16 h-16 text-[#A09F87] mx-auto mb-4 opacity-50" />
                        <h3 className="text-xl font-semibold text-white mb-2">Aucun article publié</h3>
                        <p className="text-gray-400 mb-6">Commencez à partager vos sagesses et expériences</p>
                        <button
                            onClick={() => navigate("/articla/nouveauArticle")}
                            className="bg-[#A09F87] hover:bg-[#A09F87]/80 text-[#171717] px-6 py-3 rounded-lg font-medium transition-colors flex items-center gap-2 mx-auto"
                        >
                            <Plus className="w-5 h-5" />
                            ...
                            <div className="p-6 pb-4">
                                <div className="flex items-center justify-between">
                                    ...
                                    <div className="flex items-center gap-2">
                                        ...
                                        {/* ✅ Bouton de suppression mis à jour */}
                                        <button
                                            onClick={() => handleDelete(poste.article.id)}
                                            disabled={deleteLoading[poste.article.id]}
                                            className={`p-2 rounded transition-colors ${deleteLoading[poste.article.id]
                                                    ? "text-gray-500 cursor-not-allowed"
                                                    : "text-[#A09F87] hover:text-red-500"
                                                }`}
                                            title="Supprimer l'article"
                                        >
                                            {deleteLoading[poste.article.id] ? (
                                                <div className="w-4 h-4 border-2 border-current border-t-transparent rounded-full animate-spin"></div>
                                            ) : (
                                            ...
                                            {showDropdown === poste.article.id && (
                                                <div className="absolute right-0 mt-2 w-32 bg-[#202020] border border-[#4C3163] rounded shadow-lg z-10">
                                                    <button
                                                        onClick={() => handleDelete(poste.article.id)}
                                                        disabled={deleteLoading[poste.article.id]}
                                                        className={`w-full text-left px-3 py-2 text-sm transition-colors ${deleteLoading[poste.article.id]
                                                                ? "text-gray-500 cursor-not-allowed"
                                                                : "text-red-400 hover:bg-[#4C3163]"
                                                            }`}
                                                    >
                                                        {deleteLoading[poste.article.id] ? 'Suppression...' : 'Supprimer'}
                                                    </button>
                                                </div>
                                            )}
                                        </div> */}
